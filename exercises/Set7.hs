...
data State = Start | Error | Finished | AddedEggs | AddedFlour | AddedSugar | AddedFlourSugar | Mixed
  deriving (Eq,Show)

step Start           AddEggs  = AddedEggs
step AddedEggs       AddFlour = AddedFlour
step AddedEggs       AddSugar = AddedSugar
step AddedFlour      AddSugar = AddedFlourSugar
step AddedSugar      AddFlour = AddedFlourSugar
step AddedFlourSugar Mix      = Mixed
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = head reversed :| tail reversed ++ [x]
...
passwordAllowed pw (ContainsSome chars) = any (`elem` pw) chars
...
evaluate (Multiply a1 a2) = evaluate a1 * evaluate a2
evaluate (Add a1 a2) = evaluate a1 + evaluate a2

render :: Arithmetic -> String
render (Literal n) = show n
render (Multiply a1 a2) = "(" ++ render a1 ++ "*" ++ render a2 ++ ")"
