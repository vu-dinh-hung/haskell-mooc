...
longest :: (Ord a) => [[a]] -> [a]
longest = maximumBy (\ xs ys -> compare (length xs,head ys) (length ys,head xs))
...
incrementKey key = map (\ (k,v) -> (k,v + fromIntegral (fromEnum (k == key))))
...
winner scores player1 player2
  | scoreOf player1 < scoreOf player2 = player2
  | otherwise                         = player1
    where scoreOf player = Map.findWithDefault 0 player scores
