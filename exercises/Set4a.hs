...
allEqual xs = and [x1 == x2 | (x1, x2) <- zip xs (tail xs)]
...
-- longest ls = longest' ls (head ls)
--  where
--   longest' [] longestL = longestL
--   longest' (l : ls) longestL
--     | length l < length longestL = longest' ls longestL
--     | length l > length longestL = longest' ls l
--     | otherwise =
--         if head l < head longestL
--           then longest' ls l
--           else longest' ls longestL

longest :: (Foldable t, Ord a) => t [a] -> [a]
longest = maximumBy (\l1 l2 -> compare (length l1, head l2) (length l2, head l1))
...
incrementKey target [] = []
incrementKey target ((k, v) : kvs)
  | k == target = (k, v + 1) : incrementKey target kvs
  | otherwise = (k, v) : incrementKey target kvs
...
freqs xs = foldr (Map.alter (Just . maybe 1 (\cnt -> cnt + 1))) Map.empty xs
...
maxIndex = fst . maximumBy (\(_, x) (_, y) -> compare x y) . assocs
