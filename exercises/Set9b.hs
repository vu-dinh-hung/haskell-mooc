...
prettyPrint size queens = try (1, 1) (sort queens)
  where
    try (i, j) ((r, c) : qs)
      | i == r && j == c     = 'Q' : try (nextCol (i, j)) qs
    try (i, j) qs
      | i > size             = ""
      | j > size             = '\n' : try (nextRow (i, j)) qs
      | otherwise            = '.' : try (nextCol (i, j)) qs

-- prettyPrint size coords = intercalate "\n" [[if (r, c) `elem` coords then 'Q' else '.' | c <- [1..size]] | r <- [1..size]] ++ "\n"
...
danger coord queens = or [check coord queen | queen <- queens, check <- checks]
  where
...
prettyPrint2 size queens = intercalate "\n" [[format (r, c) | c <- [1..size]] | r <- [1..size]] ++ "\n"
  where
    format (r, c) | (r, c) `elem` queens = 'Q'
                  | danger (r, c) queens = '#'
                  | otherwise            = '.'
...
backtrack []             = []
backtrack [q]            = []
backtrack (q1 : q2 : qs) = nextCol q2 : qs
