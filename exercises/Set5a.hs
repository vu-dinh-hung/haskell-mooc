...
prettyPrint bin = prettyPrint' bin ""
  where prettyPrint' End     s = s
        prettyPrint' (I bin) s = prettyPrint' bin ('1' : s)
        prettyPrint' (O bin) s = prettyPrint' bin ('0' : s)

fromBin :: Bin -> Int
fromBin End     = 0
fromBin (O bin) = 2 * fromBin bin
fromBin (I bin) = 2 * fromBin bin + 1

toBin :: Int -> Bin
toBin n
  | n == 0         = O End
  | n == 1         = I End
  | n `mod` 2 == 1 = I (toBin $ n `div` 2)
  | otherwise      = O (toBin $ n `div` 2)
