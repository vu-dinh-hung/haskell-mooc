...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [k ^ p | p <- [0 ..]]
...
joinToLength l strs = filter ((l ==) . length) [s1 ++ s2 | s1 <- strs, s2 <- strs]
...
interpreter commands = i' commands 0 0
 where
  i' [] x y = []
  i' ("up" : commands) x y = i' commands x (y + 1)
  i' ("down" : commands) x y = i' commands x (y - 1)
  i' ("left" : commands) x y = i' commands (x - 1) y
  i' ("right" : commands) x y = i' commands (x + 1) y
  i' ("printX" : commands) x y = show x : i' commands x y
  i' ("printY" : commands) x y = show y : i' commands x y
  i' (_ : commands) x y = "invalid operation!" : i' commands x y
