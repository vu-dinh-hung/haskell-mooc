...
joinToLength len strs = [s | x <- strs, y <- strs, let s = x ++ y, length s == len]
...
interpreter commands = interpreter' commands 0 0
  where interpreter' [] _ _ = []
        interpreter' (command : rest) x y = case command of
          "up"      -> interpreter' rest x (y + 1)
          "down"    -> interpreter' rest x (y - 1)
          "left"    -> interpreter' rest (x - 1) y
          "right"   -> interpreter' rest (x + 1) y
          "printX"  -> show x : interpreter' rest x y
          "printY"  -> show y : interpreter' rest x y
          _         -> "ERROR" : interpreter' rest x y
